...
export default function VerificacaoCodigo({
  ...
  useEffect(() => {   
    if (codigoEmail) validaCodigo(codigoEmail);
  }, [codigoEmail]);
  ...
  const validaCodigo = (codigoInput: string) => {
    // Verifica se o conteúdo colado contém apenas números
    if (!/^\d*$/.test(codigoInput)) return;

    // Pega apenas os primeiros 6 caracteres
    const digitos = codigoInput.slice(0, tamanhoCodigo).split("");
